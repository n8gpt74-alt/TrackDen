  ...
  return (
    <Card className="overflow-hidden rounded-[28px] border border-[var(--app-border)] bg-[var(--app-surface-strong)] p-5 shadow-[0_24px_60px_rgba(15,23,42,0.12)]">
      <div className="mb-4 flex items-center justify-between">
        ...
        {initialDraft?.receipt_id ? (
          <span className="rounded-full bg-[var(--app-accent)]/10 px-3 py-1 text-xs font-semibold text-[var(--app-accent)]">OCR draft</span>
        ) : null}
      </div>
      ...
      <form className="space-y-4" onSubmit={handleSubmit}>
        <div className="grid grid-cols-2 gap-3">
          <button
            className={`rounded-2xl px-4 py-3 text-sm font-semibold transition ${formState.type === 'expense' ? 'bg-[var(--app-accent)] text-[var(--app-accent-text)]' : 'bg-black/5 text-[var(--app-text)] dark:bg-white/5'}`}
          ...
          <button
            className={`rounded-2xl px-4 py-3 text-sm font-semibold transition ${formState.type === 'income' ? 'bg-[var(--app-accent)] text-[var(--app-accent-text)]' : 'bg-black/5 text-[var(--app-text)] dark:bg-white/5'}`}
        ...
        <div className="grid grid-cols-[1fr_96px] gap-3">
          <input
            className="rounded-2xl border border-[var(--app-border)] bg-transparent px-4 py-3 text-lg font-semibold outline-none focus:border-[var(--app-accent)]"
            inputMode="decimal"
            min="0"
          ...
          />
          <input
            className="rounded-2xl border border-[var(--app-border)] bg-transparent px-4 py-3 text-center font-semibold uppercase outline-none focus:border-[var(--app-accent)]"
            maxLength={3}
        ...
        <select
          className="w-full rounded-2xl border border-[var(--app-border)] bg-transparent px-4 py-3 outline-none focus:border-[var(--app-accent)]"
        ...
        <textarea
          className="min-h-24 w-full rounded-2xl border border-[var(--app-border)] bg-transparent px-4 py-3 outline-none focus:border-[var(--app-accent)]"
        ...
        <div className="flex gap-3">
          <Button className="flex-1 rounded-2xl" disabled={isSubmitting} large type="submit">
            {isSubmitting ? 'Сохраняем...' : isEditing ? 'Обновить' : 'Добавить'}
          </Button>
          {isEditing ? (
            <Button className="rounded-2xl" clear large onClick={onCancelEdit} type="button">
              Отмена
            </Button>
          ) : null}
        </div>
